...
function setChoiceX() {
    var radioElement = document.querySelector("input[name='X']:checked");
    ...
    if (radioElement && textElement) {
        document.getElementById("input_X").value = radioElement.value;
        document.getElementById("input_Y").value = textElement.value;
        var canvas = document.getElementById('coordinateCanvas');
        var ctx = canvas.getContext('2d');
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        drawCoordinateAxes();
